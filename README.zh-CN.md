...
当前项目版本为 `0.1.1`。已有的 `v0.1.0` 标签曾触发失败的发布流程，现必须保持
不变：不得删除、移动或复用。`v0.1.1` 是第一个可发布的恢复版本，其标签必须指向
通过 CI 的 `main` 提交。
...
第二个可选参数用于选择 GPU 开销：`auto`（默认，根据当前上下文占用自动选择
24/36/48 步）、`low`（固定 24 步）、`balanced`（固定 36 步）或 `high`
（固定 48 步）。例如：
...
`auto` 在上下文占用低于 35% 时使用 24 步，35% 到低于 75% 时使用 36 步，
达到 75% 后使用 48 步。演示模式仍默认使用 `high`，保证录制画质稳定。
...
Warp Metal 渲染器（平滑后的 0.0 到 1.0 uniform）
  1. 无效果时直接把正常终端场景渲染到 drawable
  2. 有效果时渲染到复用的离屏 BGRA 纹理
  3. 逐像素积分近场 Schwarzschild 光子路径
  4. 添加终端透镜、多次吸积盘穿越和相对论光照
  5. 输出最终合成画面
```
...
补丁只修改 Warp 的 macOS Metal 后端。效果激活时，正常终端先绘制到复用的离屏
纹理，第二个全屏 pass 再根据所选效果运行 shader：
...
占用值为零时，窗口保持 Warp 原本的事件驱动刷新，直接把 Scene 渲染到 drawable，
不为该帧使用离屏纹理，也不执行 celestial composite pass；淡出稳定后还会释放
此前保留的离屏纹理。占用值大于零时，为了让 shader 运动，窗口会按照显示器刷新率
重绘。激活路径会复用离屏纹理并在动画帧之间保留 Scene 缓存；Warp 不扫描文件系统，
Pane 本地数值直接随它本来就要渲染的 Scene 到达 Metal 后端。
...
正常使用推荐默认的 `auto`，强调省电时可选 `low`，录制演示可用 `high`；
关闭自定义应用或结束焦点 Claude 会话即可停止刷新。
